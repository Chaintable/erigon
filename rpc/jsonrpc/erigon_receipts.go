...
// erigon_GetLatestLog is the maximum number of log can be retrieved
const GetLatestLogMaxLogCount = 30000

// erigon_GetLatestLog is the maximum number of block can be retrieved
const GetLatestLogMaxBlockCount = 1000
...
func (api *ErigonImpl) GetLogsByHash(ctx context.Context, hash common.Hash) ([][]*types.Log, error) {
	...
	if chainConfig.Bor != nil {
		...
		if len(events) != 0 {
			...
			// Validate post-Madhugiri state-sync transactions
			if chainConfig.Bor.IsMadhugiri(blockNum) {
				if len(block.Transactions()) == 0 {
					return nil, fmt.Errorf("no transactions in block %x", block.Hash())
				}
			...
			// Append state-sync logs as a separate entry
			logs = append(logs, borReceipt.Logs)
		}
	}
...
// if LogCount is specified returns the logCount records starting from latest tx of end block
...
func (api *ErigonImpl) GetLatestLogs(ctx context.Context, crit filters.FilterCriteria, logOptions filters.LogFilterOptions) (types.ErigonLogs, error) {
	...
	if logOptions.LogCount > GetLatestLogMaxLogCount {
		logOptions.LogCount = GetLatestLogMaxLogCount
	}
	if logOptions.BlockCount > GetLatestLogMaxBlockCount {
		logOptions.BlockCount = GetLatestLogMaxBlockCount
	}

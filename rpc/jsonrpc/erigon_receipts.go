...
func (api *ErigonImpl) GetLatestLogs(ctx context.Context, crit filters.FilterCriteria, logOptions filters.LogFilterOptions) (types.ErigonLogs, error) {
	...
	userProvidedCountLimit := logOptions.LogCount != 0 || logOptions.BlockCount != 0
	if !userProvidedCountLimit {
	...
	if !userProvidedCountLimit && api.blockRangeLimit != 0 && (end-begin) > uint64(api.blockRangeLimit) {
